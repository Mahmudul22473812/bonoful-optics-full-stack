...
async function seed() {
  ...
  for(const account of accounts){const email=z.email().parse(process.env[`SEED_${account.prefix}_EMAIL`]).toLowerCase();const password=z.string().min(12).parse(process.env[`SEED_${account.prefix}_PASSWORD`]);const passwordHash=await hash(password,{type:argon2id,memoryCost:65536,timeCost:3,parallelism:1});await db.user.upsert({where:{email},create:{email,name:account.name,roleId:account.roleId,passwordHash,verifiedAt:new Date()},update:{name:account.name,roleId:account.roleId,passwordHash,active:true,verifiedAt:new Date()}});}
